/*
 * train_cartpole_gaad.c — CartPole 1-10 Solver using GAAD Optimizer + Geometric Encoder
...
static void geo_encoder_forward(const GeometricEncoder* enc, const float* input, float* output) {
    int max_buf_size = 0;
    for (int i = 1; i <= enc->num_layers; ++i) {
    ...
    for (int layer = 0; layer < enc->num_layers; ++layer) {
        ...
        // Copy curr to prev for next layer
...
static int gg_trainer_init(GGTrainer* gg, int num_envs, int num_poles, int rollout_len) {
    ...
    gg->cfg.lr = 3e-4f;
    gg->cfg.epochs_per_iter = 1;
    gg->cfg.minibatch_size = 256;
...
static float gg_trainer_iter(GGTrainer* gg, uint64_t rng_state[2]) {
    ...
    for (int epoch = 0; epoch < gg->cfg.epochs_per_iter; ++epoch) {
        ...
        while (bear_sampler_next(&sampler, traj, &mb_obs, &mb_actions,
            ...
            // Recompute logprobs for continuous actions
            ...
            for (int i = 0; i < policy->num_layers; ++i) {
                ...
                if (p && p->grad.data && p->weight.data) {
                    total_params += (int)bear_tensor_numel(&p->grad);
                }
            }
    ...
    // Periodic evaluation on standard CartPole-v1
...
void gg_trainer_destroy(GGTrainer* gg) {
    if (gg->env) bear_env_close(gg->env);
...
int main(int argc, char** argv) {
    ...
    for (int poles = 1; poles <= 10; ++poles) {
        printf("\n=== Solving %d-pole CartPole (cartpole8 physics) ===\n", poles);
        
        bear_env_close(gg.env);
        if (poles == 1) {
            // 1-pole: use standard CartPole-v1 (force_mag=10) for verification
            gg.env = bear_env_create(BEAR_ENV_CARTPOLE, 1, num_envs, 4, 2, 1, &gg.global_arena);
            // Recreate encoders for 4D obs
            gg.policy_encoder = geo_encoder_create(&gg.global_arena, 4, 128, 4);
            gg.value_encoder = geo_encoder_create(&gg.global_arena, 4, 64, 3);
        } else {
