/*
 * test_knockover_reset.c — Test the knock-over reset strategy
 * Start upright, kick to fall, observe fallen state
 */
...
static void test_env_init(int nenv, int npoles) {
    ...
    if (r != 0) { fprintf(stderr, "MuJoCo init failed: %d\n", r); return; }
    printf("[TEST] Initialized %d-pole, %d envs\n", npoles, nenv);
}

static void test_env_close(void) {
...
static void simulate_fall_steps(int env_id, int steps, float kick_force) {
...
static void test_knockover_reset(int env_id) {
    int N = g_env.npoles;
    
    /* Phase 1: Start upright (victory) */
    g_env.cart_x[env_id]  = ((float)rand()/RAND_MAX-0.5f)*0.1f;
    g_env.cart_vx[env_id] = 0.0f;
    for (int p = 0; p < N; ++p) {
        ATH(env_id, p) = 0.0f;      /* θ=0 = upright */
        AOM(env_id, p) = 0.0f;
    }
    printf("[RESET] Initial: theta1=%.4f (upright)\n", ATH(env_id, 0));
    
    /* Phase 2: Random kick */
    float kick_direction = (((float)rand()/RAND_MAX) > 0.5f) ? 1.0f : -1.0f;
    ...
    simulate_fall_steps(env_id, fall_steps, kick_force);
    ...
    printf("[RESET] Fallen: theta1=%.4f (%.1f°), kick=%.1f, steps=%d\n", 
           ATH(env_id, 0), ATH(env_id, 0)*180/M_PI, kick_force, fall_steps);
}

static void test_step(float* forces, int B) {
    int N = g_env.npoles;
    float rewards[MAX_ENVS] = {0};
    uint8_t dones[MAX_ENVS] = {0};
    for (int e = 0; e < B; ++e) {
...
int main(void) {
    ...
    test_env_init(4, 3);  /* 3 poles, 4 envs */
    ...
    for (int test = 0; test < 5; ++test) {
        printf("\n--- Test %d ---\n", test + 1);
        for (int e = 0; e < 4; ++e) {
            test_knockover_reset(e);
        }
        
        /* Step a few times to see if they fall further or recover */
        float forces[MAX_ENVS] = {0};
        for (int step = 0; step < 10; ++step) {
            test_step(forces, 4);
            printf("  Step %d: theta=[%.3f, %.3f, %.3f, %.3f]\n", step,
