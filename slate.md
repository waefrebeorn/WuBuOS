...
| **Code gaps closed (2026-07-09):** anticheat `kernel_load`/`kernel_unload` (BATTLESHIP #14/#15) now real `finit_module`/`delete_module` syscalls returning genuine errno (was hardcoded -1+fprintf); `kernel_loaded` -> `stat(/sys/module/<name>)`; 14-check `test_anticheat`. Screenshot `to_clipboard` (BATTLESHIP #20) now a real zlib-backed in-memory PNG encoder (`png_encode_rgba`) storing a decodable PNG in a module clipboard buffer; `test_screenshot_clipboard` PIL-validated (100x100 RGBA). Board ~25->~22 code-level gaps. | code | BATTLESHIP.md Part 1 #14/#15/#20 |
