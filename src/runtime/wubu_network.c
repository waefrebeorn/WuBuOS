...
static void gen_id(char *out, size_t len, const char *prefix) {
    ...
    uint64_t t = now_ms();
    uint64_t c = ++counter;
    snprintf(out, len, "%s%08lx%04lx", prefix, (unsigned long)(t & 0xFFFFFFFFFFFF), (unsigned long)(c & 0xFFFF));
}
...
static void profile_default(WubuNetworkProfile *p, const char *name, WubuNetworkMode mode) {
    ...
    p->mode = mode;
    p->ipam_mode = WUBU_IPAM_POOL;
    p->enable_dns = (mode == WUBU_NET_BRIDGE);
    p->enable_ipv6 = false;
    p->enable_icmp = true;
    p->enable_qos = false;
    p->enable_mdns = false;
    p->enable_dns_sd = false;
    strncpy(p->default_policy_in, "ACCEPT", sizeof(p->default_policy_in));
    strncpy(p->default_policy_out, "ACCEPT", sizeof(p->default_policy_out));
    strncpy(p->default_policy_fwd, "DROP", sizeof(p->default_policy_fwd));
    p->created = false;
    p->enabled = true;
