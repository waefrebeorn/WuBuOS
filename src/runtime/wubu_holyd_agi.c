/*
 * wubu_holyd_agi.c -- Live ring-0 compiler AGI layer (TempleOS "God compiler")
 *
 * See wubu_holyd_agi.h. Owns an in-process holyd daemon + persistent
 * "default" session; bridges the HolyD Terminal and the AGI to the real
 * HolyD JIT compiler. Every AGI eval is disclosed to EDR.
 */
...
static int do_eval(const char *src, char *out, size_t out_size, int log_to_edr) {
    if (!g_agi_holyd) wubu_holyd_agi_init();
    if (!g_agi_holyd) {
        snprintf(out, out_size, "HolyD AGI layer not initialized");
        return -1;
    }
