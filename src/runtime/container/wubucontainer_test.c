...
int main(void) {
    ...
    assert(rc == 0);
    assert(wubu_container_registered_count(engine) == 1);
    printf("Registered handler '%s' (count=%d)\n", h.name, wubu_container_registered_count(engine));
    ...
    assert(rc == 0);
    assert(wubu_container_registered_count(engine) == 2);
    printf("Registered handler '%s' (count=%d)\n", h2.name, wubu_container_registered_count(engine));

    /* The registry actually stored the descriptors. */
    assert(strncmp(wubu_container_registered_name(engine, 0), "FFmpeg", 7) == 0);
    assert(strncmp(wubu_container_registered_name(engine, 1), "ImageMagick", 11) == 0);
