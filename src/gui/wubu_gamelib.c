...
static int compare_games(const void *a, const void *b) {
    const GameLibraryEntry *ga = (const GameLibraryEntry*)a;
    const GameLibraryEntry *gb = (const GameLibraryEntry*)b;
...
int wubu_gamelib_build_start_menu(void) {
    ...
    /* Get all games */
    GameLibraryEntry **games;
    int count;
    wubu_gamelib_list_games(games, &count, NULL);
    
    /* Add to start menu under "Games" category */
    for (int i = 0; i < count; i++) {
        GameLibraryEntry *g = games[i];
        
        if (g->status != GAME_STATUS_INSTALLED) continue;
        
        /* TODO: Add to start menu when startmenu API is available */
        (void)g;
    }
...
void wubu_gamelib_clear_start_menu(void) {
    /* Would remove game entries from start menu */
    /* For now, placeholder */
}