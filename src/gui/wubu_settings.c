...
static JsonToken g_json_tokens[JSON_MAX_TOKENS];
static int g_json_token_count = 0;
static int g_json_parse_error = 0;
...
static JsonToken *json_parse_value(const char **p);
...
static void json_free(JsonToken *root) {
    json_free_recursive(root);
    g_json_token_count = 0;
}

static JsonToken *json_find(JsonToken *obj, const char *key) {
