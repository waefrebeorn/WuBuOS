/*
 * wubu_settings_io.c -- WuBuOS settings: JSON (de)serialization
 *
 * Self-contained concern split out of wubu_settings.c: the JSON-lite write
 * helpers, the per-section savers, and the file load/save entry points.
 * Operates on the shared state (g_settings, g_settings_dirty) declared in
 * wubu_settings_internal.h. No default factories, no lifecycle/accessors.
 */
