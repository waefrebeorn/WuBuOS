/* wubu_clipboard_wl.c -- WuBuOS Clipboard Wayland/X11 protocol transport
 *
 * Extracted from wubu_clipboard.c (the Wayland data-device + primary-selection
 * listeners and drag-and-drop handlers, ~350 LOC).  This is the protocol/
 * transport concern; the in-memory clipboard store + public API stay in
 * wubu_clipboard.c.  Shares ClipboardState + helpers via the internal header.
 * Compiled only in non-test (real Wayland) builds.
 */
