...
typedef struct { int n, N; double *M, *G, *rhs, *A, *b, *k1, *k2, *k3, *k4, *qt, *qdt, *th_sums, *thd_sums; } EOMContext;
...
static void ctx_destroy(EOMContext* c) {
    free(c->M); free(c->G); free(c->rhs); free(c->A); free(c->b);
    free(c->k1); free(c->k2); free(c->k3); free(c->k4); free(c->qt); free(c->qdt);
...
static void st_eom(const StateNA* s, double u, double* qdd, EOMContext* c) {
    ...
    double *M = c->M, *G = c->G, *rhs = c->rhs, *A = c->A, *b = c->b;
    ...
    rhs[0] = u; for (int i = 1; i < N; ++i) rhs[i] = -G[i];
...
static double swing_ctrl(const StateNA* s, double Et) { double E=st_energy(s), dE=E-Et, th1=s->q[1], thd1=s->qd[1], x=s->q[0], xd=s->qd[0], k=15; if(fabs(thd1)<1e-6) return (x>0) ? -5.0 : 5.0; double sgn = (thd1 >= 0) ? 1.0 : -1.0; double u = k * dE * cos(th1) * sgn - 10.0 * x - 2.0 * xd; if(u>CP_U_MAX)u=CP_U_MAX; if(u<-CP_U_MAX)u=-CP_U_MAX; return u; }
...
int main() {
    int poles = 7;
    printf("=== %d-Pole Swing-Up ===\n", poles);
    StateNA* st = st_create(poles);
    LQR* lqr = lqr_create(poles);
    double Et = 5.0;  /* Target HIGHER energy to pump energy IN for swing-up */
